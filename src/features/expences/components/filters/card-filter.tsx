...
export function CardFilter({
  ...
  const allOptions =
    cardsResponse?.data?.data.map((card) => ({
      label: `${card.name}${card.company ? ` - ${card.company.name}` : ''} (${card.card_type})`,
      value: card.id,
      cardName: card.name,
  ...
  const filterOptions = searchable
    ? allOptions.filter((option) => {
        ...
        return (
          option.cardName.toLowerCase().includes(searchLower) ||
          option.companyName?.toLowerCase().includes(searchLower)
        )
      })
    : allOptions
